...
GameLog = collections.namedtuple(
    "GameLog",
    [
        "level",
        "name",
...
def simulate_game_outcome(
    ...
    # reaches game_to.
        ...
        expected_score = convert_ratings_to_expected_game_score(
            team_a_rating, team_b_rating, game_to, rating_diff_to_victory_margin
        )

        p = (
            expected_score.team_a
            * 1.0
            / (expected_score.team_a + expected_score.team_b)
    ...
    #     P_a_offense to be slightly higher.
        ...
        actual_score_this_time = simulate_double_negative_binomial_game(
            game_to, p_a_offense=p_a_offense, p_b_offense=p_b_offense
        )
        team_a_score = actual_score_this_time.team_a
        team_b_score = actual_score_this_time.team_b
...
class Game:
    ...
    def play_game(
        ...
        if not self.played:
            ...
            # Actually play game
            team_a_wins, score_here = simulate_game_outcome(
                self.team_a.rating,
                self.team_b.rating,
                rating_diff_to_victory_margin=rating_diff_to_victory_margin,
                method=method,
    ...
    for i, p_a_offense in enumerate(possible_pa_values):
        for j, rating_diff in enumerate(X_train["RatingDiff"].values):
            ...
            for _ in range(n_simulations):
                team_a_wins, _ = simulate_game_outcome(
                    team_a_rating,
                    team_b_rating,
                    rating_diff_to_victory_margin=rating_diff_to_victory_margin,
                    method="double negative binomial",
