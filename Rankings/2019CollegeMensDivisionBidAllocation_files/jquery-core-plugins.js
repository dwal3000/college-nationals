 ...
 * 
 * Plugin uses height for IE6 instead of min-height
 *
 */
...
/************************
jQuery.betterEqualHeights v1.0
Copyright (c) 2010 Joe Kaiser

************************/


jQuery.fn.betterEqualHeights = function(params) {
	var curList = $(this);
	curList.each(function() {
		var individualList = $(this);
		var curLi = individualList.children(':first');
		var prevCurIndex = 0;
		while(curLi.is(':not(:last-child)')) {
			curLi = curLi.next();
			var curIndex = curLi.parent().children().index(curLi);
			var curTopOffset = curLi.offset().top - parseFloat(curLi.css('margin-top'));
			var nextTopOffset;
			if(!(curLi.is(':last-child'))) {
				nextTopOffset = curLi.next().offset().top - parseFloat(curLi.next().css('margin-top'));
			}
			else {
				nextTopOffset = 0;
			}
			if((curLi.is(':last-child')) || (curTopOffset !== nextTopOffset)) {
				var individualListItems = individualList.children().filter(':lt(' + (curIndex + 1) + ')');
				if(prevCurIndex > 0) {
					individualListItems = individualListItems.filter(':gt(' + prevCurIndex + ')');
				}
				var rowHeight = 0;
				individualListItems .each(function() {
					var thisLi = $(this);
					if(thisLi.outerHeight(true) > rowHeight) {
						rowHeight = thisLi.outerHeight(true);
					}
				});
				individualListItems.each(function() {
					var padMarBor = $(this).outerHeight(true) - $(this).height();
					$(this).height(rowHeight - padMarBor);
				});
